...
$maxRows_rsUniversidades = 100;
$pageNum_rsUniversidades = 0;
if (isset($_GET['pageNum_rsUniversidades'])) {
  $pageNum_rsUniversidades = $_GET['pageNum_rsUniversidades'];
}
$startRow_rsUniversidades = $pageNum_rsUniversidades * $maxRows_rsUniversidades;
...
$query_limit_rsUniversidades = sprintf("%s LIMIT %d, %d", $query_rsUniversidades, $startRow_rsUniversidades, $maxRows_rsUniversidades);
$rsUniversidades = mysql_query($query_limit_rsUniversidades, $conColoquio) or die(mysql_error());
$row_rsUniversidades = mysql_fetch_assoc($rsUniversidades);

if (isset($_GET['totalRows_rsUniversidades'])) {
  $totalRows_rsUniversidades = $_GET['totalRows_rsUniversidades'];
} else {
  $all_rsUniversidades = mysql_query($query_rsUniversidades);
  $totalRows_rsUniversidades = mysql_num_rows($all_rsUniversidades);
}
$totalPages_rsUniversidades = ceil($totalRows_rsUniversidades/$maxRows_rsUniversidades)-1;
?>
<!DOCTYPE html>
...
<div id="div_hdr_path">&nbsp;Inicio &gt; Administrador &gt; Universidades</div>
<!-- InstanceEndEditable --><!-- InstanceBeginEditable name="Contenido" -->
<div id="div_contenido">
  <table width="90%" border="0" align="center">
    <tr>
      <td><h1 class="H_Estilo1">Administrar universidades</h1></td>
    </tr>
    <?php if ($totalRows_rsUniversidades == 0) { // Show if recordset empty ?>
      <tr>
        <td><h2 class="H_Estilo2">No hay universidades para mostrar</h2></td>
      </tr>
      <?php } // Show if recordset empty ?>
    <?php if ($totalRows_rsUniversidades > 0) { // Show if recordset not empty ?>
      <tr>
        <td align="center"><h2 class="H_Estilo2">Mostrando universidades <?php echo ($startRow_rsUniversidades + 1) ?> al <?php echo min($startRow_rsUniversidades + $maxRows_rsUniversidades, $totalRows_rsUniversidades) ?> de <?php echo $totalRows_rsUniversidades ?></h2></td>
      </tr>
      <?php } // Show if recordset not empty ?>
<tr>
      <td align="center"><h3><a href="univ_add.php" class="estilo3">Agregar nueva universidad</a></h3></td>
    </tr>
    <tr>
      <td align="center"><table width="750" class="TablaListaInventario">
        <tr>
          <th scope="col">ID</th>
        ...
        <?php do { ?>
          <tr>
            <td align="center"><?php echo $row_rsUniversidades['id_universidad']; ?></td>
            <td><?php echo $row_rsUniversidades['universidad']; ?></td>
            <td align="center"><a href="univ_edit.php?id_universidad=<?php echo $row_rsUniversidades['id_universidad']; ?>" class="estilo3">Editar</a> | <a href="univ_delete.php?id_universidad=<?php echo $row_rsUniversidades['id_universidad']; ?>" class="estilo3">Eliminar</a></td>
          </tr>
          <?php } while ($row_rsUniversidades = mysql_fetch_assoc($rsUniversidades)); ?>
      </table></td>
    </tr>
    <tr>
      <td align="center"><h3><a href="univ_add.php" class="estilo3">Agregar nueva universidad</a></h3></td>
