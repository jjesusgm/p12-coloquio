...
if (isset($_FILES['uploadedFile']) && $_FILES['uploadedFile']['error'] === UPLOAD_ERR_OK) {
  ...
  $binario_nombre_temporal=$_FILES['uploadedFile']['tmp_name'] ;
  
  // leer del archvio temporal .. el binario subido.
  // "rb" para Windows .. Linux parece q con "r" sobra ...
  $_POST['fImagen'] = addslashes(fread(fopen($binario_nombre_temporal, "rb"), filesize($binario_nombre_temporal)));
  
  // Obtener del array FILES (superglobal) los datos del binario .. nombre, tabamo y tipo.
  $_POST['fImagenNombre'] = $_FILES['uploadedFile']['name'];
  $_POST['fImagenPeso'] = $_FILES['uploadedFile']['size'];
  $_POST['fImagenTipo'] = $_FILES['uploadedFile']['type'];
}else{
  $_POST['fImagen'] = NULL;
}
?>
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  $insertSQL = sprintf("INSERT INTO noticias (id_autor, fecha, estatus, imagen, imagen_nombre, imagen_peso, imagen_tipo, titulo, subtitulo, noticia) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
                       GetSQLValueString($_POST['fAutor'], "text"),
                       GetSQLValueString($_POST['fFecha'], "date"),
                       GetSQLValueString($_POST['fEstatus'], "text"),
                       "'".$_POST['fImagen']."'",
                       //GetSQLValueString($_POST['fImagen'], "text"),
                       GetSQLValueString($_POST['fImagenNombre'], "text"),
                       GetSQLValueString($_POST['fImagenPeso'], "text"),
                       GetSQLValueString($_POST['fImagenTipo'], "text"),
...
if (isset($_SESSION['MM_Username'])) {
  $colname_rsAutor = $_SESSION['MM_Username'];
}
mysql_select_db($database_conColoquio, $conColoquio);
$query_rsAutor = sprintf("SELECT * FROM usuarios WHERE username = %s", GetSQLValueString($colname_rsAutor, "text"));
$rsAutor = mysql_query($query_rsAutor, $conColoquio) or die(mysql_error());
...
<div id="div_hdr_path">&nbsp;Inicio &gt; Administrar noticias</div>
<!-- InstanceEndEditable --><!-- InstanceBeginEditable name="Contenido" -->
<div id="div_contenido">
  <table width="90%" align="center">
    <tr>
      <td><h1 class="H_Estilo1">Agregar una noticia</h1></td>
    </tr>
    <tr>
      <td align="center"><form action="<?php echo $editFormAction; ?>" method="POST" enctype="multipart/form-data" name="form1">
        <div class="DivShadowMsg">
          <table width="800" class="tabla_info_msg">
            <tr>
              <th colspan="2" align="left" scope="col">Nueva noticia</th>
              </tr>
            <tr>
              <td align="right"><label for="fAutor">Autor</label></td>
              <td><input name="fAutor" type="text" id="fAutor" value="<?php echo $row_rsAutor['username']; ?>" size="10" maxlength="20" readonly>
                <?php echo $row_rsAutor['nombres']." ".$row_rsAutor['ape_pat']." ".$row_rsAutor['ape_mat']; ?></td>
            </tr>
            <tr>
              <td align="right"><label for="fFecha">Fecha</label></td>
              <td><input name="fFecha" type="text" id="fFecha" value="<?php echo date("Y-m-d H:i:s"); ?>" size="20" maxlength="20" readonly></td>
            </tr>
            <tr>
            ...
            <tr>
              <td align="right"><label for="fImagen">Imagen</label></td>
              <td><input name="uploadedFile" type="file" id="uploadedFile" size="60">
                <input type="hidden" name="fImagen" id="fImagen">
                <input type="hidden" name="fImagenNombre" id="fImagenNombre">
                <input type="hidden" name="fImagenPeso" id="fImagenPeso">
                <input type="hidden" name="fImagenTipo" id="fImagenTipo"></td>
            </tr>
            <tr>
              <td align="right" valign="top"><label for="fTitulo">Título</label></td>
              <td><textarea name="fTitulo" cols="85" rows="2" required id="fTitulo"></textarea><span class="validity"></span></td>
            </tr>
            <tr>
              <td align="right" valign="top"><label for="fSubtitulo">Subtítulo</label></td>
              <td><textarea name="fSubtitulo" cols="85" rows="2" id="fSubtitulo"></textarea><span class="validity"></span></td>
            </tr>
            <tr>
              <td align="right" valign="top"><label for="fNoticia">Noticia</label></td>
              <td><textarea name="fNoticia" cols="85" rows="4" id="fNoticia" required></textarea><span class="validity"></span></td>
            </tr>
            <tr>
