...
<?php include_once("../includes/manejo_archivos.php") ?>
<?php
...
<div id="div_hdr_path">&nbsp;Inicio &gt; Soporte &gt; Ver archivos</div>
<!-- InstanceEndEditable --><!-- InstanceBeginEditable name="Contenido" -->
<div id="div_contenido">
  <table width="90%" align="center">
    <tr>
      <td><h1 class="H_Estilo1 margin_both">Ver archivos (pagos, ponencias, presentaciones)</h1></td>
    </tr>
    <tr>
      <td><table width="100%">
        <tr>
          <td>- <a href="ver_archivos.php?tipo=Pagos" class="estilo3">Comprobantes de pago</a></td>
        </tr>
        <tr>
          <td>- <a href="ver_archivos.php?tipo=Ponencias" class="estilo3">Ponencias</a></td>
        </tr>
        <tr>
          <td>- <a href="ver_archivos.php?tipo=Presentaciones" class="estilo3">Presentaciones</a></td>
        </tr>
        <tr>
          <td>- <a href="ver_archivos.php?tipo=CartasAceptacion" class="estilo3">Cartas de aceptación</a></td>
        </tr>
      </table></td>
    </tr>
    <tr>
      <td>
<?php
//obtener_estructura_directorios("../inscripcion/comp_pago");
...
$files = NULL;
if (isset($_GET['tipo'])){
	$tipo_archivos = $_GET['tipo'];
	switch ($tipo_archivos){
		case "Pagos":
			$files = glob("../inscripcion/comp_pago/*.*");
			break;
		case "Ponencias":
			$files = glob("../lineamientos/ponencias/*.*");
			break;
		case "Presentaciones":
			$files = glob("../lineamientos/presentaciones/*.*");
			break;
		case "CartasAceptacion":
			$files = glob("../inscripcion/cartas_aceptacion/*.*");
			break;
		default:
			$files = glob("../inscripcion/comp_pago/*.*");
	}
...
if ($num_files > 0){
	echo "<h3 class='H_Estilo2 margin_top'>Lista de archivos de ".$tipo_archivos."</h3>";
	echo "<table class='TablaListaInventario'>";
	echo "<tr><th></th><th>Tipo</th><th width='200'>Nombre del archivo</th><th width='100'>Tamaño</th></tr>";
	foreach($files as $nombre_archivo){
		$pathinfo = pathinfo($nombre_archivo);
		echo "<tr>";
		echo "<th></th>";
		echo "<td align='center'><img src='../imagenes/icono_" . $pathinfo['extension'] . ".png' style='max-width:20px'></td><td><a href='".$nombre_archivo."' target='_blank' class='estilo3'>".basename($nombre_archivo)."</a></td><td align='right'>" . FileSizeConvert(filesize($nombre_archivo)) . "</td>";
		echo "</tr>";
	}
	echo "</table>";
}else{
