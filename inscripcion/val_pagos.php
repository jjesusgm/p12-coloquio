...
$maxRows_rsComprobantes = 10;
$pageNum_rsComprobantes = 0;
if (isset($_GET['pageNum_rsComprobantes'])) {
  $pageNum_rsComprobantes = $_GET['pageNum_rsComprobantes'];
}
$startRow_rsComprobantes = $pageNum_rsComprobantes * $maxRows_rsComprobantes;

mysql_select_db($database_conColoquio, $conColoquio);
$query_rsComprobantes = "SELECT * FROM usuarios WHERE comp_pago_stat = 'Subido' ORDER BY ape_pat ASC";
$query_limit_rsComprobantes = sprintf("%s LIMIT %d, %d", $query_rsComprobantes, $startRow_rsComprobantes, $maxRows_rsComprobantes);
$rsComprobantes = mysql_query($query_limit_rsComprobantes, $conColoquio) or die(mysql_error());
$row_rsComprobantes = mysql_fetch_assoc($rsComprobantes);

if (isset($_GET['totalRows_rsComprobantes'])) {
  $totalRows_rsComprobantes = $_GET['totalRows_rsComprobantes'];
} else {
  $all_rsComprobantes = mysql_query($query_rsComprobantes);
  $totalRows_rsComprobantes = mysql_num_rows($all_rsComprobantes);
}
$totalPages_rsComprobantes = ceil($totalRows_rsComprobantes/$maxRows_rsComprobantes)-1;
?>
<!DOCTYPE html>
...
<div id="div_hdr_path">&nbsp;Inicio &gt; Inscripción &gt; Validacion de pagos</div>
<!-- InstanceEndEditable --><!-- InstanceBeginEditable name="Contenido" -->
<div id="div_contenido">
  <table width="90%" align="center">
    <tr>
      <td><h1 class="H_Estilo1 margin_both">Validación de pagos</h1></td>
    </tr>
    <tr>
      <td>En esta página se pueden ver todos los comprobantes de pago escaneados que se han subido al servidor y de esta manera validarlos para que se pueda continuar con el proceso. Mas adelante los ponentes tendrán que subir también el archivo de la ponencia y la presentación.</td>
    </tr>
    <?php if ($totalRows_rsComprobantes == 0) { // Show if recordset empty ?>
      <tr>
        <td><h2 class="H_Estilo2 margin_both">No hay comprobantes de pago por validar</h2></td>
      </tr>
      <?php } // Show if recordset empty ?>
    <?php if ($totalRows_rsComprobantes > 0) { // Show if recordset not empty ?>
  <tr>
    <td align="center"><h2 class="H_Estilo2">Mostrando comprobantes&nbsp;<?php echo ($startRow_rsComprobantes + 1) ?>&nbsp;al&nbsp;<?php echo min($startRow_rsComprobantes + $maxRows_rsComprobantes, $totalRows_rsComprobantes) ?>&nbsp;de&nbsp;<?php echo $totalRows_rsComprobantes ?> </h2></td>
  </tr>
      <tr>
        <td><table width="100%" class="TablaListaInventario">
          ...
          <?php do { ?>
            <tr align="center">
              <td><?php echo $row_rsComprobantes['username']; ?></td>
              <td><?php echo $row_rsComprobantes['nivel_de_acceso']; ?></td>
              <td align="left"><?php echo $row_rsComprobantes['nombres']; ?>&nbsp;<?php echo $row_rsComprobantes['ape_pat']; ?>&nbsp;<?php echo $row_rsComprobantes['ape_mat']; ?></td>
              <td><?php echo $row_rsComprobantes['sexo']; ?></td>
              <td><?php echo $row_rsComprobantes['comp_pago_stat']; ?></td>
              <td><?php echo $row_rsComprobantes['comp_pago_fecha']; ?></td>
              <td><a href="ver_archivos.php?username=<?php echo $row_rsComprobantes['username']; ?>" class="estilo3">Ver archivo</a></td>
            </tr>
            <?php } while ($row_rsComprobantes = mysql_fetch_assoc($rsComprobantes)); ?>
        </table></td>
      </tr>
      <tr>
        <td>&nbsp;</td>
...
<!-- InstanceEnd --></html>
<?php
mysql_free_result($rsComprobantes);
?>
