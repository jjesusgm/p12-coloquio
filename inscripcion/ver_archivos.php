...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE usuarios SET comp_pago_stat=%s, comp_pago_fecha=%s, comp_pago_validador=%s, comentario1=%s WHERE username=%s",
                       GetSQLValueString($_POST['fCompPagoStat'], "text"),
                       GetSQLValueString($_POST['fCompPagoFecha'], "date"),
                       GetSQLValueString($_POST['fCompPagoValidador'], "text"),
...
<div id="div_hdr_path">&nbsp;Inicio &gt; Inscripción &gt; Pagos &gt; Ver archivos</div>
<!-- InstanceEndEditable --><!-- InstanceBeginEditable name="Contenido" -->
<div id="div_contenido">
  <table width="90%" align="center">
    <tr>
      <td><h1 class="H_Estilo1">Ver archivos para validar</h1></td>
    </tr>
    <tr>
      <td>
<?php
date_default_timezone_set('America/Mexico_City');
$files = glob("./comp_pago/".$colname_rsUsuario.".*");
$num_files = count($files);

if ($num_files > 0){
	echo "<table class='TBL_MsgOk'>
			<tr>
			  <th scope='row'>&nbsp;</th>
			  <td colspan='2'>Archivos que se pueden revisar para validar:</td>
			</tr>";
	foreach($files as $nombre_archivo){
        echo "<tr>
				<th scope='row'>&nbsp;</th>
				<td width='3%'>&nbsp;</td>
				<td>- <a href='".$nombre_archivo."' target='_blank' class='estilo3'>".substr($nombre_archivo, strpos($nombre_archivo, $colname_rsUsuario))."</a></td>
			  </tr>";
	}
	echo "</table>";
}else{
	echo "<table class='TBL_MsgWarning'>
			<tr>
          	  <th scope='row'>&nbsp;</th>
          	  <td>El usuario no ha subido ningún archivo para validar.</td>
        	</tr>
      	  </table>";
}
?>
    ...
    <tr>
      <td><form action="<?php echo $editFormAction; ?>" name="form1" method="POST"><div class="DivShadowMsgValidar">
        <table width="550" align="center" class="tabla_info_msg">
          <tr>
            <th colspan="2" align="left" scope="col">¿Qué desea hacer con este comprobante de pago?</th>
          </tr>
          <tr>
          ...
          <tr>
            <td align="right"><label for="fCompPagoStat">Estatus</label></td>
          ...
          <tr>
            <td align="right"><label for="fCompPagoFecha">Fecha</label></td>
            <td><input name="fCompPagoFecha" type="text" id="fCompPagoFecha" value="<?php echo date("Y-m-d"); ?>" size="15" maxlength="10" readonly></td>
          </tr>
          <tr>
            <td align="right"><label for="fCompPagoValidador">Validó</label></td>
            <td><input name="fCompPagoValidador" type="text" id="fCompPagoValidador" value="<?php echo $row_rsValidador['username']; ?>" size="10" maxlength="20" readonly>
              <?php echo " ".$row_rsValidador['nombres']." ".$row_rsValidador['ape_pat']." ".$row_rsValidador['ape_mat']; ?></td>
          </tr>
          <tr>
            <td align="right"><label for="fComentario1">Comentario</label></td>
            <td><input name="fComentario1" type="text" id="fComentario1" size="50" maxlength="100"></td>
          </tr>
          <tr>
            <td colspan="2" align="right"><input type="submit" name="button" id="button" value="Enviar">
              <a href="val_pagos.php" class="button-link">Cancelar</a></td>
          </tr>
        </table></div>
