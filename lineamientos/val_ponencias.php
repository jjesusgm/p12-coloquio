...
$maxRows_rsPonencias = 20;
$pageNum_rsPonencias = 0;
if (isset($_GET['pageNum_rsPonencias'])) {
  $pageNum_rsPonencias = $_GET['pageNum_rsPonencias'];
}
$startRow_rsPonencias = $pageNum_rsPonencias * $maxRows_rsPonencias;

mysql_select_db($database_conColoquio, $conColoquio);
$query_rsPonencias = "SELECT * FROM usuarios WHERE arch_pon_stat = 'Subido' ORDER BY ape_pat ASC";
$query_limit_rsPonencias = sprintf("%s LIMIT %d, %d", $query_rsPonencias, $startRow_rsPonencias, $maxRows_rsPonencias);
$rsPonencias = mysql_query($query_limit_rsPonencias, $conColoquio) or die(mysql_error());
$row_rsPonencias = mysql_fetch_assoc($rsPonencias);

if (isset($_GET['totalRows_rsPonencias'])) {
  $totalRows_rsPonencias = $_GET['totalRows_rsPonencias'];
} else {
  $all_rsPonencias = mysql_query($query_rsPonencias);
  $totalRows_rsPonencias = mysql_num_rows($all_rsPonencias);
}
$totalPages_rsPonencias = ceil($totalRows_rsPonencias/$maxRows_rsPonencias)-1;

$queryString_rsPonencias = "";
if (!empty($_SERVER['QUERY_STRING'])) {
  $params = explode("&", $_SERVER['QUERY_STRING']);
  $newParams = array();
  foreach ($params as $param) {
    if (stristr($param, "pageNum_rsPonencias") == false && 
        stristr($param, "totalRows_rsPonencias") == false) {
      array_push($newParams, $param);
    }
  }
  if (count($newParams) != 0) {
    $queryString_rsPonencias = "&" . htmlentities(implode("&", $newParams));
  }
}
$queryString_rsPonencias = sprintf("&totalRows_rsPonencias=%d%s", $totalRows_rsPonencias, $queryString_rsPonencias);
?>
<!DOCTYPE html>
...
<div id="div_hdr_path">&nbsp;Inicio &gt; Lineamientos &gt; Validacion de ponencias</div>
<!-- InstanceEndEditable --><!-- InstanceBeginEditable name="Contenido" -->
<div id="div_contenido">
  <table width="90%" align="center">
    <tr>
      <td><h1 class="H_Estilo1">Validación de ponencias</h1></td>
    </tr>
    <tr>
      <td>En esta página se pueden ver todas las ponencias que han sido subidas al servidor y de esta manera validarlas para que se pueda continuar con el proceso.</td>
    </tr>
    <?php if ($totalRows_rsPonencias == 0) { // Show if recordset empty ?>
      <tr>
        <td><h2 class="H_Estilo2">No hay ponencias por validar</h2></td>
      </tr>
      <?php } // Show if recordset empty ?>
    <?php if ($totalRows_rsPonencias > 0) { // Show if recordset not empty ?>
  <tr>
    <td align="center"><h2 class="H_Estilo2">Mostrando ponencias &nbsp;<?php echo ($startRow_rsPonencias + 1) ?> al <?php echo min($startRow_rsPonencias + $maxRows_rsPonencias, $totalRows_rsPonencias) ?> de <?php echo $totalRows_rsPonencias ?></h2></td>
  </tr>
      <tr>
        <td align="center"><table>
          <tr>
            <td scope="row">Navegación:</td>
            <td><a href="<?php printf("%s?pageNum_rsPonencias=%d%s", $currentPage, 0, $queryString_rsPonencias); ?>" class="estilo3">Primera</a></td>
            <td>|</td>
            <td><a href="<?php printf("%s?pageNum_rsPonencias=%d%s", $currentPage, max(0, $pageNum_rsPonencias - 1), $queryString_rsPonencias); ?>" class="estilo3">Anterior</a></td>
            <td>|</td>
            <td><a href="<?php printf("%s?pageNum_rsPonencias=%d%s", $currentPage, min($totalPages_rsPonencias, $pageNum_rsPonencias + 1), $queryString_rsPonencias); ?>" class="estilo3">Siguiente</a></td>
            <td>|<a href="<?php printf("%s?pageNum_rsPonencias=%d%s", $currentPage, $totalPages_rsPonencias, $queryString_rsPonencias); ?>" class="estilo3">Ultima</a></td>
            <td>&nbsp;</td>
          </tr>
        </table></td>
      </tr>
      <tr>
        <td><table width="100%" class="TablaListaInventario">
          ...
          <?php do { ?>
            <tr align="center">
              <td><?php echo $row_rsPonencias['username']; ?></td>
              <td><?php echo $row_rsPonencias['nivel_de_acceso']; ?></td>
              <td align="left"><?php echo $row_rsPonencias['nombres']; ?>&nbsp;<?php echo $row_rsPonencias['ape_pat']; ?>&nbsp;<?php echo $row_rsPonencias['ape_mat']; ?></td>
              <td><?php echo $row_rsPonencias['sexo']; ?></td>
              <td><?php echo $row_rsPonencias['arch_pon_stat']; ?></td>
              <td><?php echo $row_rsPonencias['arch_pon_fecha']; ?></td>
              <td><a href="ver_ponencias.php?username=<?php echo $row_rsPonencias['username']; ?>" class="estilo3">Ver archivo</a></td>
            </tr>
            <?php } while ($row_rsPonencias = mysql_fetch_assoc($rsPonencias)); ?>
        </table></td>
      </tr>
