...
if (isset($_POST['uploadBtn']) && $_POST['uploadBtn'] == 'Subir al servidor') {
	...
	if (isset($_FILES['uploadedFile']) && $_FILES['uploadedFile']['error'] === UPLOAD_ERR_OK) {
        ...
        $allowedfileExtensions = array('ppt', 'pptx', 'zip');
		
		if (in_array($fileExtension, $allowedfileExtensions)) {
        	// Directorio al que se debe mover el archivo que se subio
            $uploadFileDir = './presentaciones/';
            ...
            if(move_uploaded_file($fileTmpPath, $dest_path)) {
				...
				$pres_pon_status = "Subido";
				date_default_timezone_set('America/Mexico_City');
				$current_date = date('Y-m-d');
				
				$sql = "UPDATE usuarios ".
						"SET pres_pon_stat = '$pres_pon_status', pres_pon_fecha = '$current_date' ".
    ...
    header('Location: pres_ponencia.php');
}
?>